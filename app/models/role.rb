...
class Role < ApplicationRecord
  ...
  def self.duplicate_name(name, provider)
    RESERVED_ROLE_NAMES.include?(name.downcase) || Role.exists?(["lower(name) = ? AND provider = ?", name.downcase, provider])
  end

  def self.create_default_roles(provider)
    Role.find_or_create_by(name: "user", provider: provider, priority: 1, colour: "#868e96")
      .update_all_role_permissions(can_create_rooms: true)
    Role.find_or_create_by(name: "admin", provider: provider, priority: 0, colour: "#f1c40f")
      .update_all_role_permissions(can_create_rooms: true, send_promoted_email: true, send_demoted_email: true, can_edit_site_settings: true, can_manage_rooms_recordings: true, can_edit_roles: true, can_manage_users: true)
    Role.find_or_create_by(name: "pending", provider: provider, priority: -1, colour: "#17a2b8").update_all_role_permissions
    Role.find_or_create_by(name: "denied", provider: provider, priority: -2, colour: "#343a40").update_all_role_permissions
    Role.find_or_create_by(name: "super_admin", provider: provider, priority: -3, colour: "#cd201f")
      .update_all_role_permissions(can_create_rooms: true, send_promoted_email: true, send_demoted_email: true, can_edit_site_settings: true, can_edit_roles: true, can_manage_users: true, can_manage_rooms_recordings: true)
  end
