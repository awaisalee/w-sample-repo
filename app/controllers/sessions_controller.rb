...
class SessionsController < ApplicationController
  ...
  def create
    ...
    return redirect_to(signin_path, alert: I18n.t("invalid_login_method", omniauth: user.try(:provider))) unless user.greenlight_account?
  ...
  def omniauth
    ...
    begin
      ...
      # TODO: Omniauth failed to be fixed
      omniauth_fail
    end
  end
  ...
  def switch_account_to_social
    user = User.find_by(email: @auth['info']['email'], provider: @auth[:provider], social_uid: nil)
