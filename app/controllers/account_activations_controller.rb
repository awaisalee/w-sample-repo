...
class AccountActivationsController < ApplicationController
  ...
  before_action :ensure_unauthenticated, except: [:resend, :edit]
  ...
  def edit
    ...
    if @user && !@user.activated?
      ...
      WelcomeEmailJob.set(wait: 24.hours).perform_later(@user.id) if ENV['HUBSPOT_EMAIL_ENABLED']
      ...
      redirect_to home_page, flash: { alert: I18n.t("verify.invalid") }
    end
  end
  ...
  def ensure_unauthenticated
    redirect_to home_page if current_user.present?
  end
end
