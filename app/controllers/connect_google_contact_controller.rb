class ConnectGoogleContactController < ApplicationController
  def redirect
  ...
  def save_token(response)
    token = current_user.tokens.find_or_initialize_by(provider: 'google_contact')
    ...
    current_user.update(integrated_google_contact: true) if token.save
  end
  ...
  def client_options
    {
      ...
      redirect_uri: contact_callback_url
    }
  end
end
